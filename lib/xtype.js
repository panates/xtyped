...
XArray.prototype.extend = function(schema, name, typeDef) {
  ...
  // Set items as self items
  inst.items = this.items || anyType;

  // Change items if defined in typeDef
  var t = (typeDef && typeDef.items);
  if (t) {
    ...
    if (t instanceof XType)
      ...
      t = def.base === 'array' ? def.items : def.base;
      var gen;
...
XArray.prototype._decode = function(value, varargs) {
  ...
  const items = this.items || anyType;
  const args = Array.prototype.slice.call(arguments, 1);
  value.forEach(function(v, idx) {
    value[idx] = items.decode.apply(items, [v].concat(args));
  });
  return value;
};
...
XArray.prototype._encode = function(value, varargs) {
  ...
  const args = Array.prototype.slice.call(arguments, 1);
  value.forEach(function(v, idx) {
    value[idx] = items.encode.apply(items, [v].concat(args));
  });
  return value;
};
...
XArray.prototype._validate = function(value, varargs) {
  ...
  if (!Array.isArray(value))
    throw new ValidateError('Validation error for `%s`. `%s` is not a valid array value',
        this.name, value);
  // minOccurs
...
XObject.prototype.extend = function(schema, name, typeDef) {
  ...
  const getGenericType = function(name, value, genericAliases, extendArgs) {
    ...
    const isArray = def.base === 'array';
    var t = isArray ? def.items : def.base;
    ...
    t = def.base === 'array' ? def.items : def.base;
    if (extendArgs) {
      t = extendArgs[gen.index];
      // merge t to gen
      if (t) {
        t = parseTypeDefinition(name + (isArray ? 'Item' : ''), t);
        merge({
          deep: true,
          filter: function(k, v) {
            return !(v === undefined);
    ...
    if (isArray)
      def.items = gen; else def.base = gen;
